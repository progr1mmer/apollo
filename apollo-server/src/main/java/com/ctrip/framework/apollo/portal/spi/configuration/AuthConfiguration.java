  ...
  static class SpringSecurityAuthAutoConfiguration {
    ...
    public JdbcUserDetailsManager jdbcUserDetailsManager(AuthenticationManagerBuilder auth,
      ...
      JdbcUserDetailsManager jdbcUserDetailsManager = auth.jdbcAuthentication()
          ...
          .usersByUsernameQuery("select Username,Password,Enabled from `PortalUsers` where Username = ?")
          .authoritiesByUsernameQuery(
              "select Username,Authority from `PortalAuthorities` where Username = ?")
          .getUserDetailsService();

      jdbcUserDetailsManager.setUserExistsSql("select Username from `PortalUsers` where Username = ?");
      jdbcUserDetailsManager
          .setCreateUserSql("insert into `PortalUsers` (Username, Password, Enabled) values (?,?,?)");
      jdbcUserDetailsManager
          .setUpdateUserSql("update `PortalUsers` set Password = ?, Enabled = ? where id = (select u.id from (select id from `Users` where Username = ?) as u)");
      jdbcUserDetailsManager.setDeleteUserSql("delete from `PortalUsers` where id = (select u.id from (select id from `Users` where Username = ?) as u)");
      jdbcUserDetailsManager
          .setCreateAuthoritySql("insert into `PortalAuthorities` (Username, Authority) values (?,?)");
      jdbcUserDetailsManager
          .setDeleteUserAuthoritiesSql("delete from `PortalAuthorities` where id = (select u.id from (select id from `Users` where Username = ?) as u)");
      jdbcUserDetailsManager
          .setChangePasswordSql("update `PortalUsers` set Password = ? where id = (select u.id from (select id from `Users` where Username = ?) as u)");
  ...
  static class SpringSecurityConfigurer extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
      ...
      http.authorizeRequests()
          .antMatchers("/prometheus/**","/metrics/**", "/openapi/**", "/vendor/**", "/styles/**", "/scripts/**", "/views/**", "/img/**", "/i18n/**",
                  "/", "/eureka/**",
                  "/services/**", "/configs/**", "/notifications/**").permitAll()
          .antMatchers("/**").hasAnyRole(USER_ROLE);
      http.formLogin().loginPage("/signin").defaultSuccessUrl("/index.html", true).permitAll().failureUrl("/signin?#/error").and()
          .httpBasic();
