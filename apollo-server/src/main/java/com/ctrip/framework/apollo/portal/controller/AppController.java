...
public class AppController {
  ...
  @GetMapping
  public List<PortalApp> findApps(@RequestParam(value = "appIds", required = false) String appIds) {
  ...
  @GetMapping("/search")
  public PageDTO<PortalApp> searchByAppIdOrAppName(@RequestParam(value = "query", required = false) String query,
      Pageable pageable) {
  ...
  @GetMapping("/by-owner")
  public List<PortalApp> findAppsByOwner(@RequestParam("owner") String owner, Pageable page) {
    Set<String> appIds = Sets.newHashSet();

    List<PortalRole> userRoles = rolePermissionService.findUserRoles(owner);

    for (PortalRole role : userRoles) {
  ...
  @PostMapping
  public PortalApp create(@Valid @RequestBody AppModel appModel) {

    PortalApp app = transformToApp(appModel);

    PortalApp createdApp = appService.createAppInLocal(app);
  ...
  public void update(@PathVariable String appId, @Valid @RequestBody AppModel appModel) {
    ...
    PortalApp updatedApp = appService.updateAppInLocal(app);
  ...
  @GetMapping("/{appId:.+}")
  public PortalApp load(@PathVariable String appId) {
  ...
  public void deleteApp(@PathVariable String appId) {
    PortalApp app = appService.deleteAppInLocal(appId);
  ...
  private PortalApp transformToApp(AppModel appModel) {
