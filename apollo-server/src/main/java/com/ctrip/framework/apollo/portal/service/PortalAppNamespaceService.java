...
import com.ctrip.framework.apollo.portal.repository.PortalAppNamespaceRepository;
...
@Service
public class PortalAppNamespaceService {
  ...
  private final PortalAppNamespaceRepository appNamespaceRepository;
  ...
  public PortalAppNamespaceService(
      final UserInfoHolder userInfoHolder,
      final PortalAppNamespaceRepository appNamespaceRepository,
      final PortalRoleInitializationService roleInitializationService,
      final @Lazy PortalAppService appService,
   ...
   */
  public List<PortalAppNamespace> findPublicAppNamespaces() {
    return appNamespaceRepository.findByIsPublicTrue();
  }

  public PortalAppNamespace findPublicAppNamespace(String namespaceName) {
    List<PortalAppNamespace> appNamespaces = appNamespaceRepository.findByNameAndIsPublic(namespaceName, true);
  ...
  private List<PortalAppNamespace> findAllPrivateAppNamespaces(String namespaceName) {
    return appNamespaceRepository.findByNameAndIsPublic(namespaceName, false);
  }

  public PortalAppNamespace findByAppIdAndName(String appId, String namespaceName) {
    return appNamespaceRepository.findByAppIdAndName(appId, namespaceName);
  }

  public List<PortalAppNamespace> findByAppId(String appId) {
  ...
  public void createDefaultAppNamespace(String appId) {
    ...
    PortalAppNamespace appNs = new PortalAppNamespace();
  ...
  public PortalAppNamespace createAppNamespaceInLocal(PortalAppNamespace appNamespace) {
    return createAppNamespaceInLocal(appNamespace, true);
  }

  @Transactional
  public PortalAppNamespace createAppNamespaceInLocal(PortalAppNamespace appNamespace, boolean appendNamespacePrefix) {
    String appId = appNamespace.getAppId();

    //add app org id as prefix
    PortalApp app = appService.load(appId);
    if (app == null) {
    ...
    PortalAppNamespace createdAppNamespace = appNamespaceRepository.save(appNamespace);
  ...
  private void checkAppNamespaceGlobalUniqueness(PortalAppNamespace appNamespace) {
    checkPublicAppNamespaceGlobalUniqueness(appNamespace);

    List<PortalAppNamespace> privateAppNamespaces = findAllPrivateAppNamespaces(appNamespace.getName());

    if (!CollectionUtils.isEmpty(privateAppNamespaces)) {
      Set<String> appIds = Sets.newHashSet();
      for (PortalAppNamespace ans : privateAppNamespaces) {
  ...
  private void checkPublicAppNamespaceGlobalUniqueness(PortalAppNamespace appNamespace) {
    PortalAppNamespace publicAppNamespace = findPublicAppNamespace(appNamespace.getName());
  ...
  @Transactional
  public PortalAppNamespace deleteAppNamespace(String appId, String namespaceName) {
